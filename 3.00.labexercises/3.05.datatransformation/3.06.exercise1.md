...
检查写模块要素类型参数。单击标记为表名的字段（请记住这个标签是特定于格式的而在MapInfo中我们处理的是"表格"）并将名称从Park更改为ParksMaintenanceData：
...
|  警察局长Webb-Mapp说...... |
| :--- |
|  一些写模块属性（ParkArea和AverageParkArea）有红色连接箭头，因为没有任何内容可以映射到它们，而另一个（邻居）只是未连接。  没关系。如果你保证以后联系他们，我会谨慎地告诉你。您仍然可以运行此工作空间，以查看输出的内容。 |
