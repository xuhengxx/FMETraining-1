...
单击确定两次。您将看到“选择要素类”对话框，因为我们的读模块数据集包含多个图层。我们只需要名为Neighborhoods的要素类，它包含街区边界的多边形。确保“街区”是所选的唯一要素类：
...
但是，我们的读模块要素类只包含多边形，因此我们可以通过以下方式清理起始工作空间：

* 删除GeometryFilter
* 删除Neighborhoods\_point，Neighborhoods\_line和Neighborhoods\_geom写模块要素类
* 将Neighborhoods\_polygon 写模块要素类重命名为“Neighborhoods”
...
首先，将“工作流选项”从“单个要素类”（默认）更改为“单个合并要素类”。Excel读模块将行视为要素和工作表作为要素类。这意味着默认情况下，FME会尝试将文件中的每个工作表作为单独的要素类进行读取。如果您检查数据，您会发现每个街区有一个表。这适用于Excel，但我们希望我们的数据库将所有公共艺术品视为一个点层。选择单合并要素为我们做到这一点。
...
单击“确定”之前，单击“参数”。我们必须确保读模块识别Excel工作表中的正确列作为坐标。在Attributes表下，名为Longitude的C列应为x\_coordinate类型，名为Latitude的列D应为y\_coordinate类型。您的对话框应如下所示：
...
单击确定两次。这会在我们的导航器窗口中添加一个新的Excel读模块，并在我们的画布上为其提供一个名为&lt;All&gt;的要素类，这是合并要素类的默认名称。然而，这不是很具描述性。让我们添加一个注释来表明这是合并的公共艺术品数据。右键单击要素类，然后选择“附加注记”。你可以写一些像“公共艺术品安装地点（点）”的东西：
...
即使我们添加了来自所有街区的公共艺术品安装地点，我们仍然可以通过fme\_feature\_type属性区分街区。此属性仅提供要素所属的要素类的名称。它存在于所有FME要素上，但并不总是暴露在外。每当您添加合并的要素类时，它都会默认公开。如果单击新&lt;All&gt;读模块要素类旁边的下拉箭头，则可以看到它。

如果右键单击此要素类并选择“检查”，则可以看到数据仍按街区进行组织。如果选择要素，则“要素信息”窗口中显示的第一个属性是“FME要素类”，它将显示街区名称。

让我们将fme\_feature\_type属性转换为对我们的地理数据库更有意义的属性。在&lt;All&gt; 读模块要素类后添加AttributeManager。打开参数编辑器，然后单击“输出属性”列中的“属性操作”表。这将允许您重命名该属性。我们将其更改为NeighborhoodName而不是fme\_feature\_type。“操作”列将自动更改为“重命名”。您的属性操作表现在应如下所示：
...
**4）为公共艺术品添加写模块要素类**  
现在我们需要一个写模块要素类用于我们的公共艺术品点。

|  技巧 |
| :--- |
|  请记住，我们已经设置了地理数据库写模块。我们不想添加另一个写模块，因为我们希望将所有数据写入同一个输出数据库。相反，我们需要使用要素类，因为我们希望我们的数据按层组织。 |

我们可以在“写模块”&gt;“添加要素类”下的菜单栏中为写模块添加要素类。因为我们只有一个写模块，所以它会被自动选中。将新要素类型命名为PublicArt，并为其指定Geometry类型geodb\_point。“要素类”对话框应如下所示：
...
单击确定。将新要素类连接到属性管理器的输出端口。你的画布应如下所示：
...
根据自行车道的长度将自行车道路数据分成三个形状文件（L表示长，M表示中等，S表示简称）。就像公共艺术品点一样，我们不需要在数据库中分离这些要素。因此，让我们将工作流选项从单个要素类型更改为单个合并要素类。因为shapefile包含坐标系信息，所以我们不需要在此更改。您的对话框应如下所示：
...
单击确定。这会为我们的画布添加另一个&lt;All&gt;要素型。同样，我们应该添加一个注释，以便清楚哪个读模块是哪个：
...
如果检查读模块要素类，您会发现数据已经具有值为S，M或L的PathType属性。因此，我们不需要使用AttributeManager重命名fme\_feature\_type，就像我们对公共艺术品数据所做的那样。但是，我们也不希望将fme\_feature\_type写入我们的最终数据。

我们可以在我们的读模块要素类上取消它。选择您的自行车路径读模块要素类，然后转到参数编辑器中的“格式属性”选项卡。您将看到已检查fme\_feature\_type。只需取消选中它，然后单击确定/应用：
...
现在，如果单击自行车路径读模块要素类上的下拉箭头，您将看到已根据需要从架构中删除了fme\_feature\_type。

  
**6）为自行车路径添加写模块要素类**  
现在我们需要一个用于自行车路径线的**写模块要素类**。单击“写模块”&gt;“添加要素类”，将新要素类命名为BikePaths，并为其指定几何类型geodb\_polyline。“要素类”对话框应如下所示：
...
单击确定。将包含自行车路径要素的所有读模块要素类连接到您的写模块要素类BikePaths。你的画布应如下所示：
...
**7）重投影数据**  
将Reprojector转换器添加到画布，然后将其连接到“街区”要素类。选择UTM83-10作为目标坐标系。右键单击Reprojector转换器，然后选择“复制”以添加另一个。在AttributeManager输出端口和Public Art写模块要素类型之间进行连接。

这将确保我们的街区和公共艺术品数据与我们的自行车道一致。你的画布应如下所示：
...
如果城市规划部想知道街区的自行车道的总长度和公共艺术品安装地点的数量怎么办？我们如何使用此工作空间执行此操作？
...
**10）计算公共艺术品的统计数据**  
在公共艺术品Reprojector之后添加StatisticsCalculator。我们将用这个来计算街区公共艺术品安装地点的数量。你的画布应如下所示：
...
单击确定/应用。Summary输出端口现在将输出一个表格，其中包含街区公共艺术品安装地点的数量。

让我们把它写成我们地理数据库中的一个表。单击“写模块”&gt;“添加要素类”。将其称为PublicArtSummaryStats并为其指定几何类型geodb\_no\_geom。这将把它存储为一个没有几何图形的表。单击确定。添加新要素类后，将其连接到StatisticsCalculator的Summary端口。你的画布应如下所示：
...
**11）计算自行车路径的统计数据**  
在画布上添加LineOnAreaOverlayer。这个转换器将让我们将街区中的属性添加到与它们重叠的自行车路径中。这将让我们知道每个自行车路径段所在的街区。Reprojector应连接到Area端口（因为它是街区的多边形），并且自行车路径读模块要素类应连接到Line端口。打开LineOnAreaOverlayer参数，然后选中“属性聚合”&gt;“合并属性”框。您的对话框应如下所示：
...
我们也将这些结果写成地理数据库中的表格。单击“写模块”&gt;“添加要素类”。称之为BikePathSummaryStats，并为其指定几何类型geodb\_no\_geom。单击确定。添加新要素类后，将其连接到自行车路径StatisticsCalculator的Summary端口。你的画布应如下所示：
...
将ChartGenerator转换器添加到画布。将其附加到公共艺术StatisticsCalculator的Summary输出端口。打开它的参数。在图表设置下，对于类型选择饼状图。标题输入：街区公共艺术品的分布，，温哥华，不列颠哥伦比亚省。在“数据系列”下，将“数据属性”设置为\_count，将“标签属性”设置为“街区名称”。将显示百分比更改为是。您的对话框应如下所示：
...
对于“添加要素类”&gt;“栅格文件定义”，请选择“手动”。我们选择这个是因为我们不希望这些图表图像映射我们工作空间中已存在的任何模式。对话框应如下所示：
...
将打开另一个对话框以指定PNG写模块的要素类。对于“栅格文件名”，请输入PublicArtChart。将栅格&gt;World文件生成更改为否。您的对话框应如下所示：
...
单击确定。现在将新的写模块要素类连接到公共艺术品ChartGenerator的输出端口。

对名为BikePathChart的新要素类重复此过程：单击Writers&gt; Add Writer（s）并确保Writer设置为PNGRASTER。将其连接到自行车道路ChartGenerator。由于这是我们将在此示例中进行的操作，因此请添加书签和/或注释以解释您的工作空间（如果您需要提醒如何执行此操作，请参阅[“样式”部分](https://github.com/safesoftware/FMETraining/blob/FME-Desktop-Data-Integration-2018/Integration3LabExercises/..%5CIntegration2LabDemonstration%5C2.24.Style.md)）。之后你的画布应该是这样的：
  ...
  <tbody>
    <tr>
      <td style="text-align:left">
        ...
        <ul>
          <li>使用单个写模块上的多个要素类将数据集成到单个数据库中</li>
...
1. 这个城市在哪里可以找到一个新的公共艺术品安装地点？首先要找出哪个街区拥有最少的公共艺术品安装地点。然后在离现有公共艺术品安装地点最远的街区里找一个市政拥有的房产\(C:\FMEData2018\Data\ \CityProperties\ cityproperty .shp\)。这是一个新安装的地点。
